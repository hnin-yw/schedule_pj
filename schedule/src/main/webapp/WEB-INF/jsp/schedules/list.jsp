		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="form-group">
					<label for="eventTitle">スケジュールタイトル :</label> <span id="eventTitle"></span>
				</div>
				<div class="form-group">
					<label for="eventStart">スケジュールの開始日時 :</label> <span id="eventStart"></span>
				</div>
				<div class="form-group">
					<label for="eventEnd">スケジュールの終了日時 :</label> <span id="eventEnd"></span>
				</div>
				<div class="form-group">
					<label for="eventMeetLink">ミーティングのリンク :</label> <span
						id="eventMeetLink"></span>
				</div>
			</div>
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-danger"
					id="btnEventAttendeeDelete" data-toggle="modal"
					data-target="#deleteEventAttendeeConfirmModel">削除</button>
				<button type="button" class="btn btn-danger" id="btnEventDelete"
...
<div class="modal fade" id="addScheduleModal" tabindex="-1"
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
					...
					<div class="row">
						<div class="col-sm-9">
							...
							<div class="form-group" style="margin-top: 10px;">
								<label for="color">スケジュールテーマカラー</label> <input type="color"
									...
									value="#FF4013" class="form-control">
							</div>
							<div class="form-group">
								<div class="col-sm-2" style="margin-top: 20px;">
									<input type="checkbox" class="custom-checkbox"
										id="allDayFlgChk"> <label for="allDayFlgChk">一日中</label>
								</div>
								<div class="col-sm-3">
								...
								<div class="form-group" id="repeateDetail">
									...
									<div class="col-sm-4" id="repeatUntilDiv" style="display: none">
										<label for="repeatUntil"> 繰り返す終了日付 :</label> <input
											type="text" id="repeatUntil" name="repeatUntil"
						...
						<div class="col-sm-3">
							<div class="panel cus_left-panel">
								<label for="isTask"> スケジュールタイプ</label> <select id="isTask"
									...
									id="otherVisibilityFlg" name="otherVisibilityFlg"
									class="form-control">
									<option value="0">公開</option>
									<option value="1">非公開</option>
								</select> <br> <label for="scheduleDisplayFlg"> スケジュール表示</label> <select
									id="scheduleDisplayFlg" name="scheduleDisplayFlg"
									class="form-control">
						...
						<div class="form-group col-sm-12 mt-link">
							<label for="meetLink"> ミーティングのリンク :</label> <input type="text"
								...
								class="form-control"> <span id="meetLinkError"
								class="error" style="color: red;"></span>
						</div>
						...
						<div class="form-group col-sm-12">
							...
							<textarea id="scheduleDescription" name="scheduleDescription"
								placeholder="スケジュールの説明" class="form-control"></textarea>
							<span id="scheduleDescriptionError" class="error"
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" id="btnMoreOptions">More
					Options</button>
				<button type="button" class="btn btn-primary" id="btnAddSchedule">登録</button>
				<button type="button" class="btn btn-secondary" data-dismiss="modal"
					id="btnAddCancel">キャンセル</button>
			</div>
		</div>
	</div>
</div>
			...
			<div class="modal-body">
				...
				<div class="form-check form-group">
					<input type="radio" class="form-check-input" name="deleteOption"
						id="deleteAll" value="1" checked> <label
						class="form-check-label" for="deleteAll">すべてのスケジュール削除</label>
				</div>
				<div class="form-check form-group">
					<input type="radio" class="form-check-input" name="deleteOption"
						id="deleteOne" value="0"> <label class="form-check-label"
						for="deleteOne">このスケジュールのみ削除</label>
				</div>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" id="btnDeleteOK">OK</button>
				<button type="button" class="btn btn-secondary" id="btnDeleteCancel"
					data-dismiss="modal">キャンセル</button>
			</div>
		</div>
	</div>
</div>
<!-- delete Attendee ConfirmModel modal -->
<div class="modal fade" id="deleteEventAttendeeConfirmModel"
	tabindex="-1" role="dialog"
	aria-labelledby="deleteEventAttendeeConfirmModelLabel"
	aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<security:csrfMetaTags />
			...
			<div class="modal-body">
				<p>招待された</p>
				<p>スケジュールを削除してもよろしいですか?</p>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" id="btnDeleteEventOK">OK</button>
				<button type="button" class="btn btn-secondary" id="btnDeleteCancel"
					data-dismiss="modal">キャンセル</button>
			</div>
		</div>
...
<script>
    $(document).ready(function() {
	    ...
	    $('#btnDeleteEventOK').click(function() {
		    var csrfToken = $("meta[name='_csrf']").attr("content");
		    var csrfHeader = $("meta[name='_csrf_header']").attr("content");
		    ...
		    $.ajax({
		        url: "/schedule/schedules/attendee/deleteById/" + scheduleId,
		        type: "DELETE",
		        beforeSend: function(xhr) {
		            xhr.setRequestHeader(csrfHeader, csrfToken);
		        },
	            success: function(response) {
		            $("#deleteEventAttendeeConfirmModel").modal("hide");
